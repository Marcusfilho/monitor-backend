...
/* PATCH_MNS_VHCLS_JOURNAL_V2
 * - Loga VHCLS_REQ/VHCLS_RES no journal (sem vazar cookie/token).
 * - Mostra head e tenta extrair vehicle_id por múltiplos padrões.
 */
(() => {
  try {
    if (globalThis.__mns_vhcls_journal_v2) return;
    globalThis.__mns_vhcls_journal_v2 = true;

    const _origFetch = globalThis.fetch;
    if (typeof _origFetch !== "function") {
      console.log("[PATCH_MNS_VHCLS_JOURNAL_V2] WARN fetch is not a function; inactive");
      return;
    }

    function _hdrGet(headers, k){
      try{
        if (!headers) return "";
        if (typeof headers.get === "function") return String(headers.get(k) || "");
        const key = String(k).toLowerCase();
        if (Array.isArray(headers)) {
          for (const [hk,hv] of headers) if (String(hk).toLowerCase()===key) return String(hv||"");
          return "";
        }
        if (typeof headers === "object") {
          for (const hk of Object.keys(headers)) if (String(hk).toLowerCase()===key) return String(headers[hk]||"");
        }
      }catch{}
      return "";
    }
    ...
    function _bodyToString(body){
      try{
        ...
        if (typeof body === "object" && typeof body.toString === "function") return String(body.toString());
      }catch{}
      return "";
    }

    function _extractVehicleId(txt){
      const pats = [
        [/VEHICLE_ID\s*=\s*"(\d+)"/i, "VEHICLE_ID_dq"],
        [/VEHICLE_ID\s*=\s*'(\d+)'/i, "VEHICLE_ID_sq"],
        [/VEHICLE_ID\s*=\s*(\d+)/i, "VEHICLE_ID_raw"],
        [/\bvehicle_id\b\s*[:=]\s*"(\d+)"/i, "vehicle_id_json_dq"],
        [/\bvehicle_id\b\s*[:=]\s*(\d+)/i, "vehicle_id_json_raw"],
        [/\b<VHCL\b[^>]*\bID\s*=\s*"(\d+)"/i, "VHCL_ID_dq"],
        [/\bvehicleId\b\s*[:=]\s*"(\d+)"/i, "vehicleId_json_dq"],
        [/\bvehicleId\b\s*[:=]\s*(\d+)/i, "vehicleId_json_raw"],
      ];
      for (const [re, tag] of pats) {
        const m = txt.match(re);
        if (m) return { id: m[1], tag };
      }
      return null;
    }

    globalThis.fetch = async function MNS_VHCLS_FETCH_V2(input, init){
      const url = (() => { try { return String(input && input.url ? input.url : input); } catch { return ""; } })();
      const bodyStr = _bodyToString(init && init.body);

      const isVhcls = /(^|[?&])action=VHCLS(&|$)/.test(bodyStr) || /\baction=VHCLS\b/.test(bodyStr);
      if (!isVhcls) return _origFetch.apply(this, arguments);

      const plate = (() => {
        try{
          const m = bodyStr.match(/(?:^|[&?])LICENSE_NMBR=([^&]+)/i);
          return m ? decodeURIComponent(m[1]).replace(/\+/g," ") : "";
        }catch{ return ""; }
      })();

      const cookie = _hdrGet(init && init.headers, "cookie") || _hdrGet(init && init.headers, "Cookie");
      const ckLen = cookie ? cookie.length : 0;

      console.log(`[VHCLS_REQ] plate=${plate||"-"} ckLen=${ckLen} bodyLen=${bodyStr.length} url=${url.slice(0,120)}`);

      const t0 = Date.now();
      const res = await _origFetch.apply(this, arguments);
      const ms = Date.now() - t0;

      let status = "<?>"; try{ status = String(res && res.status); }catch{}

      let head = ""; let len = -1; let found = null;
      try{
        if (res && typeof res.clone === "function") {
          const txt = await res.clone().text();
          len = txt.length;
          head = txt.slice(0,280).replace(/\s+/g," ").trim();
          found = _extractVehicleId(txt);
        }
      }catch(e){
        head = `[clone_failed ${String(e && (e.message||e))}]`;
      }


      const headSafe = (/ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=|EULA_APPROVED=|APPLICATION_ROOT_NODE=/i.test(head)) ? "<cookie_header_redacted>" : head;

      console.log(`[VHCLS_RES] plate=${plate||"-"} status=${status} ms=${ms} len=${len} head=${headSafe}`);
if (found) {
        console.log(`[VHCLS_PARSE] plate=${plate||"-"} vehicle_id=${found.id} via=${found.tag}`);
      } else {
        console.log(`[VHCLS_PARSE] plate=${plate||"-"} vehicle_id=NOT_FOUND`);
      }
      return res;
    };

    console.log("[PATCH_MNS_VHCLS_JOURNAL_V2] installed");
  } catch (e) {
    console.log("[PATCH_MNS_VHCLS_JOURNAL_V2] install failed:", e && (e.message || e.toString()));
  }
})();

/* PATCH_MNS_VHCLS_JOURNAL_V1 (log-only, sem /tmp)
 * - Loga VHCLS REQ/RES no journal (cookieLen, status, len, head)
 * - Nunca imprime cookie/token
 * - Safe: não quebra fluxo (try/catch)
 */
(() => {
  try {
    if (globalThis.__mns_vhcls_journal_v1) return;
    globalThis.__mns_vhcls_journal_v1 = true;

    const _origFetch = globalThis.fetch;
    if (typeof _origFetch !== "function") {
      console.log("[PATCH_MNS_VHCLS_JOURNAL_V1] WARN fetch is not a function; inactive");
      return;
    }

    let _undiciFetch = null;
    try { _undiciFetch = require("undici").fetch; } catch {}
    ...
    globalThis.fetch = async function MNS_VHCLS_FETCH(input, init){
      const url = (() => { try { return String(input && input.url ? input.url : input); } catch { return ""; } })();
      const bodyStr = _bodyToString(init && init.body);
      const isVhcls = /(^|[?&])action=VHCLS(&|$)/.test(bodyStr) || /\\baction=VHCLS\\b/.test(bodyStr);
      if (!isVhcls) return _origFetch.apply(this, arguments);
      ...
      let res;
      try{
        // se quiser isolar hook existente: export MNS_VHCLS_USE_UNDICI=1 no systemd/env
        if (process && process.env && process.env.MNS_VHCLS_USE_UNDICI === "1" && _undiciFetch) {
          res = await _undiciFetch(url, init);
        } else {
          res = await _origFetch.apply(this, arguments);
        }
      }catch(e){
        console.log(`[VHCLS_RES] plate=${plate||"-"} ERROR fetch_throw=${String(e && (e.stack||e.message||e))}`.slice(0,800));
        throw e;
      }

      const ms = Date.now() - t0;
      let status = "";
      try{ status = String(res && res.status); }catch{ status = "<?>"; }
      ...
      try{
        if (res && typeof res.clone === "function") {
          const txt = await res.clone().text();
          len = txt.length;
          head = txt.slice(0,260).replace(/\\s+/g," ").trim();
        }
      }catch(e){
        head = `[clone_failed ${String(e && (e.message||e))}]`;
      }

      console.log(`[VHCLS_RES] plate=${plate||"-"} status=${status} ms=${ms} len=${len} head=${head}`);
      return res;
    };

    console.log("[PATCH_MNS_VHCLS_JOURNAL_V1] installed");
  } catch (e) {
    console.log("[PATCH_MNS_VHCLS_JOURNAL_V1] install failed:", e && (e.message || e.toString()));
  }
})();

// === PATCH_SAFE_TEXT_ANY_V1
async function __safeTextAny(x) {
  try {
    if (x == null) return "";
    if (typeof x === "string") return x;
    if (typeof x.text === "function") return await x.text();
...
// === PATCH_NATIVE_HTTPS_POSTFORM_V1 ===
function __httpsPostFormUrlencoded(urlStr, headers, bodyStr){
  return new Promise((resolve, reject) => {
    try {
      const https = require("https");
      const { URL } = require("url");
      const u = new URL(String(urlStr));
      const body = Buffer.from(String(bodyStr||""), "utf8");
      const h = Object.assign({}, (headers||{}));
      if (!h["content-type"] && !h["Content-Type"]) h["content-type"] = "application/x-www-form-urlencoded; charset=UTF-8";
      ...
      const req = https.request(opts, (res) => {
        ...
        res.on("end", () => {
          ...
          let sc = [];
          try {
            const v = (res && res.headers) ? res.headers["set-cookie"] : null;
  ...
  if (!globalThis.__VHCLS_FETCH_TAP_INSTALLED && typeof globalThis.fetch === 'function') {
    ...
    globalThis.fetch = async function(input, init) {
      ...
      try {
        ...
        const want =
          /(^|[?&])action=VHCLS(&|$)/.test(bodyStr) ||
          (bodyStr.indexOf('action=VHCLS') >= 0);

        if (want) {
          const c = res.clone();
          const txt = await __safeTextAny(c);
...
async function _vhclsResolveVehicleIdDirect(ctx, licenseKey) {
  ...
  // garante cookiejar (warmup/login) antes de chamar VHCLS
  try { if (typeof __va_ensureHtml5Session === "function") await __va_ensureHtml5Session({ tag: "VHCLS_DIRECT_PRE" }); } catch (e) {}
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    const __jar = _readCookieHeaderFromJarFile(ctx);
    const cookieHeader = ensureCookieDefaults((__jar && __jar.cookie) ? __jar.cookie : "");
    ...
    const res = await fetch(HTML5_ACTION_URL, {
      ...
      headers: {
        ...
        "origin": (HTML5_BASE_URL || "https://html5.traffilog.com"),
        "referer": (HTML5_BASE_URL || "https://html5.traffilog.com") + "/",
    ...
    try {
      const last = { status: res.status, len: text.length, head: safeSnippet(text, 280) };
      if (ctx) ctx.__vhcls_last = { licenseKey: lk, ...last };
    } catch (e) {}

    // artefato (opcional)
    try {
      const jobId = ctx && (ctx.jobId || ctx.id || ctx.job_id);
      if (jobId) {
        const fp = `/tmp/vhcls_raw_${String(jobId)}_${Date.now()}_a${attempt}.xml`;
        require("fs").writeFileSync(fp, text, "utf8");
      }
    } catch (e) {}

    const parsed = _parseVehicleIdFromVhclsXml(text, lk);

    if (!parsed.err && parsed.vehicleId) return parsed.vehicleId;

    // Se der erro de sessão (login=-1 ou "Action: VHCLS error"), tenta 1x forçar relogin.
    if (parsed.err === "unauthorized_or_vhcls_error" && attempt === 1) {
      try {
        const jar = __va_readJarRaw();
        const mm = __va_parseJarToMap(jar);
        try { delete mm.TFL_SESSION; } catch (e) {}
        try { delete mm["ASP.NET_SessionId"]; } catch (e) {}
        __va_writeJarMap(mm);
        console.log(`[vhcls] direct ${lk}: unauthorized/action_error -> forcing relogin and retry`);
      } catch (e) {}
      try { if (typeof __va_ensureHtml5Session === "function") await __va_ensureHtml5Session({ tag: "VHCLS_DIRECT_RETRY" }); } catch (e) {}
      continue;
    }
...
const patchC8 = (() => {
  try { return require('./patchC8_allowedGroups'); }
    ...
    try { console.warn('[WARN] patchC8_allowedGroups missing; skipping allowed-groups patch'); } catch (_e) {}
    return { applyPatchC8AllowedGroups: () => ({ ok:false, skipped:true, reason:'patchC8_allowedGroups_missing' }) };
  }
...
const res = await __origFetch(url, init);
      ...
      try {
        ...
        if (__bodyStr && __bodyStr.includes('action=VHCLS')) {
          ...
          const txt = c ? await __safeTextAny(c) : '';
...
const HTML5_BASE_URL = (process.env.HTML5_BASE_URL || "").trim();
...
function __va_parseJarToMap(raw){
  ...
  return out;
        ...
        // formato map: filtra lixo/não-string
        const drop = new Set(["cookie","cookies","keys","updatedAt","createdAt","meta","id","job","payload","status","type","service","plate","workerId"]);
        const out = {};
        for (const k of Object.keys(j)) {
          if (!k || drop.has(k)) continue;
...
function __va_cookieHeaderFromMap(m){
  const obj = m || {};
  // === PATCH_ROOTNODE_URLENCODE_V1 ===
  try {
    const rv = obj && obj["APPLICATION_ROOT_NODE"];
    if (typeof rv === "string" && rv && /[{}"]/.test(rv) && rv.indexOf("%7B") === -1) {
  ...
  const parts = [];
  for (const k of ordered) parts.push(`${k}=${String(obj[k])}`);
  return parts.join("; ");
}

// === PATCH_VA1_COOKIEJAR_SANITIZE_V2 ===
// PATCH_VA1_COOKIEJAR_MERGE_EXTRAS_V3
...
// === PATCH_VA1_SEED_EULA_ROOT_V1 ===
// Seed de cookies "core" exigidos pelo HTML5 em alguns fluxos (sem vazar valores)
function __va_seedCoreCookies(mapIn){
  const m = Object.assign({}, mapIn||{});
  let changed = 0;
  if (!m["EULA_APPROVED"]) { m["EULA_APPROVED"] = "1"; changed = 1; }
  if (!m["APPLICATION_ROOT_NODE"]) { m["APPLICATION_ROOT_NODE"] = "%7B%22node%22%3A%22-2%22%7D"; changed = 1; }
  return { m, changed };
}
// === /PATCH_VA1_SEED_EULA_ROOT_V1 ===
...
async function __va_warmupAsp(){
  ...
  try {
    const res = await __va_fetch(HTML5_ACTION_URL, { method:"GET", redirect:"manual" });
...
async function __va_ensureHtml5Session(){
  ...
  // === PATCH_SEED_CORE_COOKIES_V1 (VHCLS exige EULA_APPROVED + APPLICATION_ROOT_NODE) ===

  try {

    let __changed = 0;

    if (m1 && !m1["EULA_APPROVED"]) { m1["EULA_APPROVED"] = "1"; __changed = 1; }

    if (m1 && !m1["APPLICATION_ROOT_NODE"]) { m1["APPLICATION_ROOT_NODE"] = '{"node":"-2"}'; __changed = 1; }

    if (__changed) __va_writeJarMap(m1);

  } catch(e) {}

  // === /PATCH_SEED_CORE_COOKIES_V1 ===
// seed de cookies core (EULA/ROOT) para evitar VHCLS login=-1 / Action error
  const __seed = __va_seedCoreCookies(m1);
  const m1s = __seed.m;
  if (__seed.changed) {
    __va_writeJarMap(m1s);
  }
...
async function __va_appenginePost(action, fields, tag){
  ...
  // === PATCH_VHCLS_COOKIE_NAMES_V2 (log seguro: apenas NOMES dos cookies, sem valores)
  try {
    if (String(action||"") === "VHCLS") {
      const ck = String((sess && sess.cookie) || "");
      const names = ck.split(";").map(p => (p.split("=")[0] || "").trim()).filter(Boolean);
      console.log("[VHCLS_COOKIE_NAMES]", names.join(","));
      console.log("[VHCLS_COOKIE_LEN]", ck.length);
    }
  } catch(e) {}
  // === /PATCH_VHCLS_COOKIE_NAMES_V2


    // === PATCH_VHCLS_CANON_DUMP_V2 (bypass fetch hook + log req/res + dump /tmp/vhcls_raw_<JOB>_canon.txt) ===
  let __va_fetch = fetch;
  try {
    if (String(action||"") === "VHCLS") {
      const u = require("undici");
      if (u && typeof u.fetch === "function") __va_fetch = u.fetch;
    }
  } catch(e) {}
  const __vhcls_job = (globalThis && globalThis.__CUR_JOB_ID) ? globalThis.__CUR_JOB_ID
                    : (globalThis && globalThis.__JOB_LAST && globalThis.__JOB_LAST.id) ? globalThis.__JOB_LAST.id
                    : "";
  if (String(action||"") === "VHCLS") {
    try {
      const ck = String((sess && sess.cookie) || "");
      console.log(`[VHCLS_REQ2] tag=${tag||action} job=${__vhcls_job||""} plate=${(fields||{}).LICENSE_NMBR||""} flags=${__va_flags(ck)} cookieLen=${ck.length} bodyLen=${body.length}`);
    } catch(e) {}
  }
  // === /PATCH_VHCLS_CANON_DUMP_V2 ===
  // === PATCH_VHCLS_USE_UNDICI_IN_VA1_V1 ===
  // Para VHCLS, bypass total de hooks de fetch usando undici.fetch (evita res.status undefined / "body" virar cookie header)
  const __isVHCLS = (String(action||"") === "VHCLS");
  let __vhclsFetch = null;
  if (__isVHCLS) {
    try {
      const u = require("undici");
      if (u && typeof u.fetch === "function") __vhclsFetch = u.fetch;
    } catch(e) {}
  }
  const __fetchFn = (__vhclsFetch || fetch);
  // === /PATCH_VHCLS_USE_UNDICI_IN_VA1_V1 ===


// === PATCH_VHCLS_REQUEST_UNDICI_V1 ===
  const __reqHeaders = {
    "content-type":"application/x-www-form-urlencoded; charset=UTF-8",
    "cookie": sess.cookie
  };
  // === PATCH_VHCLS_BROWSER_HEADERS_V1 ===
  let __html5Base = "";
  try { __html5Base = (new URL(HTML5_ACTION_URL)).origin; } catch(e) {}
  try {
    __reqHeaders["accept"] = "*/*";
    __reqHeaders["accept-encoding"] = "identity";
    __reqHeaders["x-requested-with"] = "XMLHttpRequest";
    __reqHeaders["user-agent"] = "monitor-backend-worker/1.0";
    if (__html5Base) {
      __reqHeaders["origin"] = __html5Base;
      __reqHeaders["referer"] = __html5Base + "/appv2/index.htm";
    }
  } catch(e) {}
  // === /PATCH_VHCLS_BROWSER_HEADERS_V1 ===
  ...
  // === PATCH_VHCLS_NATIVE_HTTPS_V2 ===
  const __looksLikeCookie = (ss) => /ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=|EULA_APPROVED=|APPLICATION_ROOT_NODE=/i.test(String(ss||""));
  try {
    const __base = String(HTML5_ACTION_URL||"").split("/").slice(0,3).join("/");
    const __h2 = Object.assign({}, __reqHeaders, {
      "accept":"*/*",
      "accept-encoding":"identity",
      "origin": __base,
      "referer": __base + "/appv2/index.htm",
      "user-agent":"monitor-backend-worker/1.0"
    });
    const rn = await __httpsPostFormUrlencoded(HTML5_ACTION_URL, __h2, body);
    status = Number((rn && rn.status) || 0);
    txt = String((rn && rn.text) || "");
    sc = (rn && rn.setCookie) ? rn.setCookie : [];
    if (status) console.log(`[VHCLS_NATIVE] http=${status} len=${(txt||"").length} cookieLike=${__looksLikeCookie(txt)?1:0}`);
    if (__looksLikeCookie(txt)) status = 0; // força fallback
  } catch(e) {
    // deixa fallback ocorrer
  }
  // === /PATCH_VHCLS_NATIVE_HTTPS_V2 ===
  // Para VHCLS, usar undici.request (não depende do global fetch / hooks)
  if (String(action||"") === "VHCLS") {
    try {
      const u = require("undici");
      if (u && typeof u.request === "function") {
        const r2 = await u.request(HTML5_ACTION_URL, { method:"POST", headers: __reqHeaders, body });
        status = Number((r2 && r2.statusCode) || 0);

        // set-cookie (pode vir string ou array)
        try {
          const h = (r2 && r2.headers) ? r2.headers : {};
    ...
    // fallback: se undici.request falhar, usa o caminho atual (pode estar hookado)
    if (!status) {
      res = await __fetchFn(HTML5_ACTION_URL, { method:"POST", headers: __reqHeaders, body });
      status = (res && res.status) ? res.status : 0;
      txt = await __safeTextAny(res).catch(()=> "");
      sc = __va_getSetCookieList(res);
    }
  } else {
    res = await __fetchFn(HTML5_ACTION_URL, { method:"POST", headers: __reqHeaders, body });
    status = (res && res.status) ? res.status : 0;
...
if (sc && sc.length) {
  ...
  // === PATCH_VHCLS_COOKIE_GUARD_V1 ===
  try {
    if (String(action||"") === "VHCLS") {
      const __t = String(txt||"");
      const __cookieLike = /ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=|EULA_APPROVED=|APPLICATION_ROOT_NODE=/i.test(__t);
      if (__cookieLike) {
        console.log(`[VHCLS_GUARD] cookie_like_response len=${__t.length} -> forcing status=0 for retry/fail`);
        status = 0;
        txt = "";
      }
    }
  ...
  // === PATCH_VHCLS_CANON_DUMP_V2_WRITE ===
  if (String(action||"") === "VHCLS") {
    try {
      const fs = require("fs");
      const job = __vhcls_job;
      if (job) {
        const safeTag = String(tag||action||"VHCLS").replace(/[^A-Za-z0-9_]+/g, "_");
        const p = `/tmp/vhcls_raw_${job}_${safeTag}.txt`;
        // === PATCH_VHCLS_DUMP_REDACT_COOKIELIKE_V1 ===
        const __tDump = String(txt||"");
        const __cookieLikeDump = /ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=/i.test(__tDump);
        const __outDump = __cookieLikeDump ? (`[REDACTED] looks_like_cookie len=${__tDump.length}`) : __tDump;
        fs.writeFileSync(p, __outDump, "utf8");
        // === /PATCH_VHCLS_DUMP_REDACT_COOKIELIKE_V1 ===
        if (/^VHCLS_CANON/i.test(String(tag||""))) {
          fs.writeFileSync(`/tmp/vhcls_raw_${job}_canon.txt`, __outDump, "utf8");
        }
      }
      const head = String(txt||"").slice(0, 140).replace(/\s+/g, " ");
      
  // PATCH_VHCLS_RES2_REDACT_V1
  const headSafe = (/ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=|EULA_APPROVED=/i.test(head)) ? "<cookie_header_redacted>" : head;
console.log(`[VHCLS_RES2] tag=${tag||action} job=${__vhcls_job||""} http=${status} len=${(txt||"").length} loginNeg=${loginNeg?1:0} head=${headSafe}`);
    } catch(e) {}
  }
  // === /PATCH_VHCLS_CANON_DUMP_V2_WRITE ===


  console.log(`[html5_v8] [VA1] ${tag||action} status=${status} len=${(txt||"").length} loginNeg=${loginNeg?1:0} jarFlags=${sess.flags}`);
  return { status, text: txt, loginNeg, vehicleId: vid, jarFlags: sess.flags };
}
...
async function __va_vhclsRefresh(plate){
  ...
  const plLower = pl.toLowerCase(); // PATCH_VHCLS_PL_LOWER_V1
    const __vhclsFields = {
    VERSION_ID: "2",
    REFRESH_FLG: "1",
  ...
  let r = await __va_appenginePost("VHCLS", __vhclsFields, "VHCLS_CANON");

    // PATCH_VA1_VHCLS_RETRY_LOGINNEG_V2 (loginNeg OR Action error -> forcing relogin + retry 1x)
  try {
    const __isActionErr = (t) => /Action:\s*VHCLS\s+error/i.test(String(t || ""));
    if (r && (r.loginNeg || __isActionErr(r.text))) {
      const why = r.loginNeg ? "loginNeg=1" : "Action error";
      console.log(`[html5_v8] [VA1] VHCLS_CANON ${why} -> forcing relogin + retry`);
      try {
        const raw = __va_readJarRaw();
      ...
      try { if (typeof __ensureTflSession === "function") await __ensureTflSession(); } catch(e) {}
            r = await __va_appenginePost("VHCLS", __vhclsFields, "VHCLS_CANON_RETRY");
}
  } catch(e) {}
...
// === PATCH_MNS_RESOLVER_SHARED_V1 (shared resolver: UNINSTALL + MNS use same VHCLS_CANON path)
async function resolveVehicleIdByPlate_VHCLS(opts){
  const plate = String((opts && opts.plate) || "").trim();
  const tag   = String((opts && opts.tag) || "VHCLS").trim() || "VHCLS";
  const jobId = (opts && (opts.jobId !== undefined) && (opts.jobId !== null)) ? String(opts.jobId) : "";

  // keep job id visible for VHCLS dumps (/tmp/vhcls_raw_<job>_canon.txt)
  try { if (jobId) globalThis.__CUR_JOB_ID = jobId; } catch(e) {}

  // best-effort cookie length (no values)
  let cookieLen = 0;
  try {
    if (typeof loadCookieJar === "function") {
      const cj = await loadCookieJar();
      const ck = (cj && cj.cookie) ? ensureCookieDefaults(cj.cookie) : "";
      cookieLen = String(ck||"").length;
    }
  } catch(e) {}

  // PATCH_VHCLS_COOKIE0_V2: if cookieLen==0, force ensureHtml5Session and recompute
  if (!cookieLen) {
    try {
      if (typeof __va_ensureHtml5Session === "function") {
        await __va_ensureHtml5Session({ tag: String(tag||"VHCLS") + "_COOKIE0" });
      }
    } catch(e) {}
    try {
      if (typeof loadCookieJar === "function") {
        const cj2 = await loadCookieJar();
        const ck2 = (cj2 && cj2.cookie) ? ensureCookieDefaults(cj2.cookie) : "";
        const after = String(ck2||"").length;
        if (after) cookieLen = after;
        console.log(`[VHCLS_COOKIE0] tag=${tag} job=${jobId} plate=${plate} cookieLenAfter=${after}`);
      }
    } catch(e) {}
  }

  console.log(`[VHCLS_REQ] tag=${tag} job=${jobId} plate=${plate} cookieLen=${cookieLen}`);
  ...
  const head = String((r && r.head) || "").slice(0, 220).replace(/\s+/g, " ");
  const headSafe = (/ASP\.NET_SessionId=|TFL_SESSION=|AWSALB=|AWSALBCORS=|EULA_APPROVED=|APPLICATION_ROOT_NODE=/i.test(head))
    ? "<cookie_header_redacted>"
    : head;

  console.log(`[VHCLS_RES] tag=${tag} job=${jobId} plate=${plate} http=${(r && r.status) || 0} len=${(r && r.len) || 0} loginNeg=${(r && r.loginNeg) ? 1 : 0} head=${headSafe}`);

  return r;
}
// === END PATCH_MNS_RESOLVER_SHARED_V1
...
async function fetchWithCookies(url, { method="GET", headers={}, body=null } = {}, cookieHeader=""){
  ...
  try{
    const res = await fetch(url, {
      ...
      let __rawText = "";
      try { __rawText = (typeof res.text === 'function' ? await __safeTextAny(res) : String(res && res.text || '')); } catch (_) {}
...
async function httpFetch(path, { method="GET", params=null, json=null } = {}) {
  ...
  try {
    ...
    const text = await __safeTextAny(res).catch(() => "");
...
function normalizeStep(step, jobPayload){
  // PATCH_NORMALIZESTEP_STRING_V1: allow string steps like "DEACTIVATE_VEHICLE_HIST"
  if (typeof step === "string") {
    const act = String(step||"").trim();
    const p = (jobPayload && typeof jobPayload === "object") ? jobPayload : {};
    const vId = p.vehicle_id || p.vehicleId || p.VEHICLE_ID || p.vehicleID || p.VehicleId;
    const plate = String(p.plate || p.LICENSE_NMBR || p.license || p.licensePlate || "").trim();
    if (act === "DEACTIVATE_VEHICLE_HIST") {
      if (!vId) throw new Error("normalizeStep string DEACTIVATE_VEHICLE_HIST missing vehicle_id");
      const installer = p.installer_name || p.installer || p.INSTALLER_NAME || "installer";
      const comments  = p.comments || p.note || p.notes || "uninstall";
      const reason    = String(p.reason_code || p.REASON_CODE || 5501);
      const deliver   = String(p.deliver_code || p.DELIVER_CODE || 5511);
      return {
    ...
    throw new Error("normalizeStep string unsupported action=" + act);
  }
...
async function main(){
  ...
  const HTML5_JOB_TYPES = String(process.env.HTML5_JOB_TYPES || process.env.JOB_TYPES || "html5_install,html5_uninstall,html5_maint_no_swap,html5_maint_with_swap,html5_change_company")
    .split(",").map(s => s.trim()).filter(Boolean);

  function serviceFromJobType_(t){
    const s = String(t || "").trim().toLowerCase();
    if (s === "html5_uninstall") return "UNINSTALL";
    if (s === "html5_install") return "INSTALL";
    if (s === "html5_maint_no_swap") return "MAINT_NO_SWAP";
    if (s === "html5_maint_with_swap") return "MAINT_WITH_SWAP";
    if (s === "html5_change_company") return "CHANGE_COMPANY";
    return "";
  }
  ...
  while(true){
    try{
      ...
      for (const jt of HTML5_JOB_TYPES) {
        const rr = await httpFetch("/api/jobs/next", { params:{ type: jt, worker:WORKER_ID } });
        if (rr.status === 204) continue;
        if (rr.status !== 200 || !rr.data) { r = null; pickedType = ""; break; }
...
const inferredService = serviceFromJobType_(job.type || job.__job_type);
      const service = normService(payload.service || payload.SERVICE || payload.servico || payload.serviceType || payload.service_type || inferredService);
      ...
      // === PATCH_JOB_CTX_V1 (propagar contexto do job atual p/ dumps/logs) ===

      try { globalThis.__CUR_JOB_ID = String(id || ""); globalThis.__CUR_SERVICE = String(service || ""); } catch(e) {}
...
try {
  if (service === "MAINT_WITH_SWAP") {
    ...
    if (!vid) {
      ...
      if (!vid) {
        vh = await __va_appenginePost("VHCLS", { VERSION_ID:"2", REFRESH_FLG: "1", LICENSE_NMBR: plate }, "MWS_VHCLS");
...
try {
  ...
  try {
    if (plate && typeof resolveVehicleIdByPlate_VHCLS === "function") {
      const __va = await resolveVehicleIdByPlate_VHCLS({ plate, tag: service || "UNINSTALL", jobId: id });
...
// [PATCH_U6] ensure required fields for DEACTIVATE_VEHICLE_HIST
      try {
        ...
        if (service === "DEBUG_BUILD_SAVE_ACTIVATION_FIELDS") {
          ...
          console.log(`[MNS] resolved vehicle_id=${__vid} job=${id} plate=${__mns_plate}`);
        ...
        if (html5ShouldSkip) {
            // === PATCH_MNS_SKIPRESOLVE_V3 (MAINT_NO_SWAP: resolve vehicle_id por placa via VHCLS_CANON; retry 1x em loginNeg/Action error; falha se não resolver)
          if (service === "MAINT_NO_SWAP") {
            const __mns_plate = String(payload && (payload.plate || payload.placa || payload.LICENSE_NMBR || payload.license || payload.licensePlate) || "").trim().toUpperCase();
            console.log(`[MNS] start job=${jobId} plate=${__mns_plate}`);
            if (!__mns_plate) {
              await finish("error", {
                ok:false,
                service,
                error:"missing_plate",
                meta:{ service, resolve_error:"missing_plate" }
              });
              return;
            }

            // resolve-only (zero mutate): reutiliza o MESMO resolver do UNINSTALL (ensureVehicleIdByVhcls_ -> VHCLS)
            const __ctx = { log: (m)=>console.log(String(m)), jobId };
            const __p = Object.assign({}, (payload || {})); // PATCH_MNS_CONST_PAYLOAD_V1
            __p.service = "MAINT_NO_SWAP";
            __p.plate = __mns_plate;

            const __vid = await ensureVehicleIdByVhcls_(__ctx, __p);
const __vh = (__ctx && __ctx.__vhcls_last) ? __ctx.__vhcls_last : null;

            if (!__vid) {
              const __head = __vh && __vh.head ? String(__vh.head) : "";
              console.log(`[MNS] resolve_failed job=${jobId} plate=${__mns_plate} head=${__head}`);
              await finish("error", {
                ok:false,
                service,
                error:"vhcls_no_vehicle_id",
                meta:{
                  service,
                  resolve_error:"vhcls_no_vehicle_id",
                  plate: __mns_plate,
                  vhcls_status: __vh && __vh.status,
                  vhcls_len: __vh && __vh.len,
                  vhcls_head: __head
                }
              });
              return;
            ...
            console.log(`[MNS] resolved job=${jobId} plate=${__mns_plate} vehicle_id=${__vid}`);
            await finish("success", {
              ...
              meta:{
                ...
                vhcls_status: __vh && __vh.status,
                vhcls_len: __vh && __vh.len,
                vhcls_head: (__vh && __vh.head) ? String(__vh.head) : ""
              }
            });
            return;
          }
          // === end PATCH_MNS_SKIPRESOLVE_V3
...
// === PATCH_VHCLS_FORCECOOKIE_FIX_V1 ===
if (!need) { return cur; }
// === /PATCH_VHCLS_FORCECOOKIE_FIX_V1 ===
