...
r.post("/vehicle-settings/bulk-upsert", requireAdmin, express_1.default.json({ limit: "2mb" }), async (req, res) => {
...
r.get("/vehicle-models", requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const nq = "%" + normKey(q) + "%";
        const out = await pool.query("select * from catalog_vehicle_models where model_key like $1 or vehicle_type_raw ilike $2 or friendly_name ilike $2 order by model_key limit 200", [nq, "%" + q + "%"]);
...
r.post("/vehicle-models/bulk-upsert", requireAdmin, express_1.default.json({ limit: "5mb" }), async (req, res) => {
    const updatedBy = (req.body?.updatedBy || "admin").toString();
    const items = Array.isArray(req.body?.items) ? req.body.items : [];
    if (!items.length)
        return res.status(400).json({ error: "items[] required" });
    ...
    try {
        ...
        for (const it of items) {
            ...
            if (!modelKey)
                throw new Error("modelKey/vehicleType vazio");
            await client.query(`
