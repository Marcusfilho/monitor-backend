...
const ASSET_TYPES_PATH = path.resolve(process.env.ASSET_TYPES_PATH ||
...
// ---------------------------------------------------------------------------
async function loginAdminToHtml5() {
    ...
    const isOk = /REDIRECT[^>]*node=-2/i.test(loginResp.body) ||
...
// ---------------------------------------------------------------------------
async function fetchUsedAssetTypeIds(adminCookie) {
    const bodyParams = new URLSearchParams({
        action: "VHCLS",
        REFRESH_FLG: "1",
        LICENSE_NMBR: "",
...
async function fetchAssetTypesCatalog(adminCookie) {
    const bodyParams = new URLSearchParams({
        action: "ASSET_TYPES",
        VERSION_ID: "2",
    });
...
router.post("/asset-types/sync", async (_req, res) => {
    try {
        ...
        const matched = catalog.filter(entry => usedIds.has(`${entry.manufacturer.toUpperCase()}|${entry.model.toUpperCase()}`));
        ...
        try {
            (0, jobStore_1.createJob)("admin_config_sync", { key: "asset_types", value: output });
    ...
    try {
        ...
        for (const p of paths) {
            try {
                ...
                for (const c of cand) {
                    if (c && typeof c?.patchInstallation === "function") {
                        store = c;
                        break;
                    }
                }
        ...
        if (!inst)
            return res.status(404).json({ ok: false, error: "installation_not_found" });
...
// ---------------------------------------------------------------------------
const SCHEMES_JSON_PATH = path.resolve(process.env.SCHEMES_PATH ||
...
function parseClientsXml(xml) {
    ...
    while ((m = regex.exec(xml)) !== null) {
        ...
        if (client_id)
            results.push({ client_id, client_descr });
    }
    return results;
}
function parseSchemesXml(xml) {
    const results = [];
...
router.post("/schemes", (req, res) => {
    try {
        ...
        // Persistir no SQLite da VM via job assíncrono
        try {
            (0, jobStore_1.createJob)("admin_config_sync", { key: "schemes_active", value: payload });
