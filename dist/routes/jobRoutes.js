...
function _alreadyHasGs(installationId) {
    try {
        return (0, jobStore_1.listJobs)().some((j) => j?.type === "monitor_gs" &&
            String(j?.payload?.installation_id ?? j?.payload?.installationId ?? "") === String(installationId) &&
            j?.status !== "error");
    }
    catch {
...
function _pickErrSummary(result) {
    try {
        if (!result)
        ...
        const code = r.code ??
            ...
            null;
        const out = { message: String(msg || "job_failed") };
        if (code != null)
            out.code = String(code);
        const hint = r.hint ?? (r.meta && r.meta.hint) ?? null;
        if (hint != null)
            out.hint = String(hint).slice(0, 240);
        return out;
    }
    catch {
        return { message: "job_failed" };
...
function _handleHtml5CompleteToInstallation(job, result, finalStatus, jobId) {
    try {
        if (!installationsStore?.getInstallation || !installationsStore?.patchInstallation)
            return;
        const type = String(job?.type || "");
        if (!(type === "html5_install" || type.startsWith("html5_")))
            return;
        const installationId = _getInstallationId(job);
        if (!installationId)
            return;
...
function _enqueueCanAfterSchemeBuilder(job, result) {
    try {
        if (!job || String(job.type || "") !== "scheme_builder")
            return;
        if (!_resultOk(result))
            return;
        const installationId = _getInstallationId(job);
        if (!installationId)
            return;
        if (_alreadyHasCan(installationId))
            return;
        const inst = installationsStore?.getInstallation ? installationsStore.getInstallation(installationId) : null;
        ...
        function _sbDoneFallback(reason) {
            console.log(`[jobs] [PIPELINE] SB concluído mas CAN não enfileirado (${reason}) — marcando SB_DONE installation=${installationId}`);
            try {
                installationsStore?.patchInstallation && installationsStore.patchInstallation(installationId, { status: "SB_DONE" });
            }
            catch { }
        }
        if (!service) {
            _sbDoneFallback("service ausente");
            return;
        }
        if (!["INSTALL", "MAINT_WITH_SWAP"].includes(service)) {
            _sbDoneFallback(`service=${service} não requer CAN`);
            return;
        }
...
function _handleGsComplete(job, result, jobId) {
    try {
        ...
        if (!installationId)
            return;
        if (!installationsStore?.patchInstallation)
            return;
...
router.post("/:id/complete", (req, res) => {
    ...
    try {
        if (job?.type === "monitor_gs")
            _handleGsComplete(job, result, id);
    }
    catch { }
    // HTML5: refletir erro/sucesso na instalação (HTML5_ERROR + last_error)
    try {
        _handleHtml5CompleteToInstallation(job, result, finalStatus, id);
    }
    catch { }
