...
import { getSessionToken, refreshSessionTokenFromDisk } from "./sessionToken";
...
// Abre conexão dedicada (sem cache) — use quando precisar de buffer isolado
export async function openFreshWs(): Promise<OpenWsResult> {
  return doConnect();
}

// Abre conexão raw sem wrappers de encode — para fluxos que enviam JSON puro
export async function openRawWs(): Promise<{ ws: any; sessionToken: string }> {
  const url = (process.env.MONITOR_WS_URL || '').trim();
  if (!url) throw new Error('MONITOR_WS_URL não definido.');
  const WebSocket = require('ws');
  const cookie  = (process.env.MONITOR_WS_COOKIE || '').trim();
  const origin  = (process.env.MONITOR_WS_ORIGIN || 'https://operation.traffilog.com').trim();
  const token   = (process.env.MONITOR_SESSION_TOKEN || '').trim();
