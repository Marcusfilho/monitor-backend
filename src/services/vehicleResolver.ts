...
export type ResolveStatus =
  | "OK"
  | "ERROR_SERIAL_ALREADY_USED"
  | "ERROR_PLATE_NOT_FOUND"
  | "ERROR_PLATE_INVALID"
  | "ERROR_SERIAL_NEW_ALREADY_USED";

export interface ResolveInstallResult {
  status: ResolveStatus;
  // Preenchidos quando status === "OK"
  vehicle_id_final?: number;
  licence_nmbr_final?: string;
  client_descr_current?: string;   // cliente atual no sistema
  client_id_current?: number;
  client_mismatch?: boolean;       // true se cliente atual != client_descr informado
  needs_uninstall_cmdt?: boolean;  // true se precisar de uninstall CMDT antes do install
  // Informações de diagnóstico
  resolution_path?: string;        // qual caminho foi percorrido
  error_message?: string;
}

export interface ResolveMaintWithSwapResult {
  status: ResolveStatus;
  // Preenchidos quando status === "OK"
  vehicle_id_final?: number;
  serial_old_found?: string;       // serial atual instalado na placa (para confirmação)
  client_descr_current?: string;
  client_id_current?: number;
  client_mismatch?: boolean;
  needs_uninstall_cmdt?: boolean;  // true se serial_new precisar de uninstall CMDT
...
export async function resolveForInstall(params: {
  licence_nmbr: string;
  serial: string;
  client_descr: string;
  ...
  let vehicle_id_final: number | null = null;
  let licence_nmbr_final: string = licence_nmbr;
  let resolution_path = "";
  ...
  if (plateRecord) {
    ...
    if (isEmptyInnerId(plateRecord.inner_id)) {
  ...
  if (vehicle_id_final === null) {
    ...
    const serialInnerRecords = await vhclsQueryBySerial(serial);
    const serialInnerRecord = serialInnerRecords.find((r) =>
      serialsMatch(r.inner_id, serial)
    ) || null;
    ...
    if (serialInnerRecord) {
      ...
      if (serialsMatch(serialInnerRecord.inner_id, serialInnerRecord.licence_nmbr)) {
  ...
  if (vehicle_id_final === null) {
    ...
    const serialLicenceRecords = await vhclsQueryByPlate(serial);
    const serialLicenceRecord = serialLicenceRecords.find(
      (r) => r.licence_nmbr.trim().toUpperCase() === serial.trim().toUpperCase()
    ) || null;
  ...
  // -------------------------------------------------------------------------
  let client_descr_current: string | undefined;
  let client_id_current: number | undefined;
  ...
  if (vehicle_id_final !== null) {
    ...
    const checkRecords = await vhclsQueryByPlate(licence_nmbr_final);
    ...
    if (checkRecord) {
      ...
      const checkBySerial = await vhclsQueryBySerial(serial);
...
export async function resolveForMaintWithSwap(params: {
  licence_nmbr: string;
  serial_old: string;
  serial_new: string;
  client_descr: string;
}): Promise<ResolveMaintWithSwapResult> {
  ...
  // -------------------------------------------------------------------------
  let vehicle_id_final: number;
  let serial_old_found: string;

  if (isEmptyInnerId(plateRecord.inner_id)) {
    // Placa sem serial instalado — aceita qualquer serial_old
    vehicle_id_final = plateRecord.vehicle_id;
    serial_old_found = "";
  } else if (serialsMatch(plateRecord.inner_id, serial_old)) {
  ...
  const newSerialRecords = await vhclsQueryBySerial(serial_new);
  const newSerialRecord = newSerialRecords.find((r) =>
    serialsMatch(r.inner_id, serial_new)
  ) || null;
  ...
  if (newSerialRecord) {
    ...
    if (serialsMatch(newSerialRecord.inner_id, newSerialRecord.licence_nmbr)) {
